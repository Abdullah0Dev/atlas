...
export default function Anibadini({ navigation}) {
    const [anibadinii,setAnibadinii] = useState([]);
    useEffect(() => {
      axios.get('https://amedbaz.github.io/anibadini/anibadini.json')
      .then(result=>{
        setAnibadinii(result.data.anibadini)
      })
      }, []);
...
{
   anibadinii.map((data,klil)=>(
      <TouchableOpacity 
   onPress={() => navigation.navigate('Dataanibadini',
    {
      wene:data.wene,
      wene1:data.wene1,
      nav:data.nav,
