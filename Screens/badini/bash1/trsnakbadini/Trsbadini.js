...
export default function Trsbadini({ navigation}) {
    const [trsbadinii,setTrsbadinii] = useState([]);
    useEffect(() => {
      axios.get('https://amedbaz.github.io/trsbadini/trsbadini.json')
      .then(result=>{
        setTrsbadinii(result.data.Trsbadini)
      })
      }, []);
...
{
   trsbadinii.map((data,klil)=>(
      <TouchableOpacity 
   onPress={() => navigation.navigate('Datatrsbadini',
    {
      wene:data.wene,
      wene1:data.wene1,
      nav:data.nav,
