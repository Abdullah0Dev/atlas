...
export default function Dramabadini({ navigation}) {
    const [dramabadinii,setDramabadinii] = useState([]);
    useEffect(() => {
      axios.get('https://amedbaz.github.io/dramabadini/dramabadini.json')
      .then(result=>{
        setDramabadinii(result.data.dramabadini)
      })
      }, []);
...
{
   dramabadinii.map((data,klil)=>(
      <TouchableOpacity 
   onPress={() => navigation.navigate('Datadramabadini',
    {
      xeleke:data.xeleke,
      wene:data.wene,
      wene1:data.wene1,
