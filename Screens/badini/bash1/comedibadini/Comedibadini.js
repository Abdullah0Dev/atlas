...
export default function Comedibadini({ navigation}) {
    const [comedibadinii,setComedibadinii] = useState([]);
    useEffect(() => {
      axios.get('https://amedbaz.github.io/comedibadini/comedibadini.json')
      .then(result=>{
        setComedibadinii(result.data.comedibadini)
      })
      }, []);
...
{
   comedibadinii.map((data,klil)=>(
      <TouchableOpacity 
   onPress={() => navigation.navigate('Datacomedibadini',
    {
      wene:data.wene,
      wene1:data.wene1,
      nav:data.nav,
