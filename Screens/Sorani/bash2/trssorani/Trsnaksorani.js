...
export default function Trsnaksorani({ navigation}) {
    const [trssorani,setTrssorani] = useState([]);
    useEffect(() => {
      axios.get('https://amedbaz.github.io/trssorani/trssoranii.json')
      .then(result=>{
        setTrssorani(result.data.Trssorani)
      })
      }, []);
...
{
   trssorani.map((data,klil)=>(
      <TouchableOpacity 
   onPress={() => navigation.navigate('Datatrsnaksorani',
    {
      wene:data.wene,
      wene1:data.wene1,
      nav:data.nav,
