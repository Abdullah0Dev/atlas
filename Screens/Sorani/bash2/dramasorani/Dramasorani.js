...
export default function Dramasorani({ navigation}) {
    const [dramasoranii,setDramasoranii] = useState([]);
    useEffect(() => {
      axios.get('https://amedbaz.github.io/dramasorani/dramasorani.json')
      .then(result=>{
        setDramasoranii(result.data.dramasorani)
      })
      }, []);
...
{
   dramasoranii.map((data,klil)=>(
      <TouchableOpacity 
   onPress={() => navigation.navigate('Datadramasorani',
    {
      xeleke:data.xeleke,
      wene:data.wene,
      wene1:data.wene1,
