...
export default function Comedisorani({ navigation}) {
    const [comedisorani,setComedisorani] = useState([]);
    useEffect(() => {
      axios.get('https://amedbaz.github.io/comedisorani/comedisorani.json')
      .then(result=>{
        setComedisorani(result.data.comedisorani)
      })
      }, []);
...
{
   comedisorani.map((data,klil)=>(
      <TouchableOpacity 
   onPress={() => navigation.navigate('Datacomedisorani',
    {
      wene:data.wene,
      wene1:data.wene1,
      nav:data.nav,
