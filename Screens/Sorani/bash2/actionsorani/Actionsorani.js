...
export default function Actionsorani({ navigation}) {
    const [actionbadinii1,setActionbadinii1] = useState([]);
    useEffect(() => {
      axios.get('https://amedbaz.github.io/actionsorani/actionsorani.json')
      .then(result=>{
        setActionbadinii1(result.data.actionsorani)
      })
      }, []);
...
{
   actionbadinii1.map((data,klil)=>(
      <TouchableOpacity 
   onPress={() => navigation.navigate('Dataactionsorani',
    {
      wene:data.wene,
      wene1:data.wene1,
      nav:data.nav,
