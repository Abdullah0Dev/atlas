...
export default function Animisorani({ navigation}) {
    const [anisorani,setAnisorani] = useState([]);
    useEffect(() => {
      axios.get('https://amedbaz.github.io/animisorani/animisorani.json')
      .then(result=>{
        setAnisorani(result.data.anisorani)
      })
      }, []);
...
{
   anisorani.map((data,klil)=>(
      <TouchableOpacity 
   onPress={() => navigation.navigate('Dataanimisorani',
    {
      wene:data.wene,
      wene1:data.wene1,
      nav:data.nav,
