...
export default function Aniarabic({ navigation}) {
    const [animiarabicc,setAnimiarabicc] = useState([]);
    useEffect(() => {
      axios.get('https://amedbaz.github.io/animiarabic/animiarabic.json')
      .then(result=>{
        setAnimiarabicc(result.data.Animiarabic)
      })
      }, []);
...
useEffect(() => {
  ...
  return (
    ...
    {
       animiarabicc.map((data,klil)=>(
          <TouchableOpacity 
       onPress={() => navigation.navigate('Dataanimiarabic',
        {
          wene:data.wene,
          wene1:data.wene1,
          nav:data.nav,
