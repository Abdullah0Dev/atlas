...
export default function Trsarabic({ navigation}) {
    const [trsarabicc,setTrsarabicc] = useState([]);
    useEffect(() => {
      axios.get('https://amedbaz.github.io/trsarabic/trsarabic.json')
      .then(result=>{
        setTrsarabicc(result.data.Trsarabic)
      })
      }, []);
...
useEffect(() => {
  ...
  return (
    ...
    {
       trsarabicc.map((data,klil)=>(
          <TouchableOpacity 
       onPress={() => navigation.navigate('Datatrsarabic',
        {
          wene:data.wene,
          wene1:data.wene1,
          nav:data.nav,
