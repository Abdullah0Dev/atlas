...
export default function Actionarabic1({ navigation}) {
    const [actionarabicc,setActionarabicc] = useState([]);
    useEffect(() => {
      axios.get('https://amedbaz.github.io/actionarabic/actionarabic.json')
      .then(result=>{
        setActionarabicc(result.data.actionarabic)
      })
      }, []);
...
{
   actionarabicc.map((data,klil)=>(
      <TouchableOpacity 
   onPress={() => navigation.navigate('Dataactionarabic',
    {
      wene:data.wene,
      wene1:data.wene1,
      nav:data.nav,
