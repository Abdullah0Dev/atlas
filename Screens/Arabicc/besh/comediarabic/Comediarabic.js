...
export default function Comediarabic({ navigation}) {
    const [comediarabicc,setComediarabicc] = useState([]);
    useEffect(() => {
      axios.get('https://amedbaz.github.io/comediarabic/comediarabic.json')
      .then(result=>{
        setComediarabicc(result.data.Comediarabic)
      })
      }, []);
...
useEffect(() => {
  ...
  return (
    ...
    {
       comediarabicc.map((data,klil)=>(
          <TouchableOpacity 
       onPress={() => navigation.navigate('Datacomediarabic',
        {
          wene:data.wene,
          wene1:data.wene1,
          nav:data.nav,
